...
export async function buildLink(type, id, data) {
  ...
  const link = await dynamicLinks().buildShortLink({
    link: `https://play.google.com/store/apps/details?id=com.letsdevelopit.lfydcustomer&type=${type}&itemId=${id}`,
...
export async function buildReferalLink(referralCode,referralText) {
  try {
    const link = await dynamicLinks().buildShortLink({
      link: `https://play.google.com/store/apps/details?id=com.letsdevelopit.lfydcustomer&referalCode=${referralCode}`,
      // domainUriPrefix is created in your Firebase console
      domainUriPrefix: 'https://fydo.page.link',
      // domainUriPrefix: 'https://lfydnewapp.page.link',
      // optional setup which updates Firebase analytics campaign
